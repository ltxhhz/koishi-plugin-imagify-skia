  ...
  background: Schema.intersect([
    ...
    Schema.union([
      ...
      Schema.object({
        ...
        colorStop: Schema.array(Schema.tuple([Schema.number().min(0).max(1).step(0.01).default(0).role('slider').description('offset'), Schema.string().description('color')]).description('[偏移量,颜色]')).description('是时候学习了 [CanvasGradient.addColorStop](https://developer.mozilla.org/zh-CN/docs/Web/API/CanvasGradient/addColorStop)')
      }),
      Schema.object({
